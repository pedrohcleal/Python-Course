...
def on_open(ws):
    ...
    client.chat_postMessage(channel='#btc-30min', text="Sinalizador-bot entrou em operação")
    ...
    if is_candle_closed:
        ...
        if len(closes) > RSI_PERIOD:
            ...
            if last_rsi > RSI_OVERBOUGHT:
                ...
                client.chat_postMessage(channel='#btc-30min',
                                        text=":botão_up: O RSI está sobrecomprado, bom momento para short"
                                             ", RSI: {:.2f}, preço ${:.2f} :botão_up: ".format(last_rsi, close))
                in_position = True
            if last_rsi < RSI_OVERSOLD:
                print("Oversold! Buy! Buy! Buy!")
                client.chat_postMessage(channel='#btc-30min',
                                        text=":seta_para_baixo: O RSI está sobrevendido, bom momento para long"
                                             ", RSI:{:.2f}, preço = ${:.2f} :seta_para_baixo:".format(last_rsi, close))
                in_position = True
            if RSI_OVERSOLD < last_rsi and RSI_OVERBOUGHT > last_rsi:
                if in_position:
                    print("RSI ENTROU EM ZONA NEUTRA")
                    client.chat_postMessage(channel='#btc-30min', text="O indicador RSI entrou em zona neutra"
