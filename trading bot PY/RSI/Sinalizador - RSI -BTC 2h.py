...
SOCKET = "wss://stream.binance.com:9443/ws/btcusdt@kline_2h"
env_path = Path('.') / '.env'
...
def on_open(ws):
    ...
    client.chat_postMessage(channel='#btc-2hr', text="Sinalizador-bot entrou em operação")
    ...
    if is_candle_closed:
        ...
        if len(closes) > RSI_PERIOD:
            ...
            if last_rsi > RSI_OVERBOUGHT:
                ...
                client.chat_postMessage(channel='#btc-2hr',
                                        text=":botão_up: O RSI está sobrecomprado, bom momento para short"
                                             ", RSI: {}, preço ${} :botão_up: ".format(last_rsi,close))
                in_position = True
            ...
            if RSI_OVERSOLD < last_rsi and RSI_OVERBOUGHT > last_rsi:
                if in_position:
                    ...
                    client.chat_postMessage(channel='#btc-2hr', text="O indicador RSI entrou em zona neutra"
                                                                     ", RSI: {:.2f}, preço ${:.2f} :botão_up: ".format(last_rsi,close))
                    in_position = False
                else:
                    print("bot ok")
